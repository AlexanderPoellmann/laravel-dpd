...
abstract readonly class DpdService implements AdditionalService
{
    public function validateProduct(Product1|string $product): void
    {
        ...
        if ($knownProduct?->isPrimetime()) {
            throw new InvalidArgumentException('This DPD additional service is not documented for primetime products.');
        }
    }
    ...
    public function validateParcelType(ParcelType $type): void
    {
        if ($type === ParcelType::Primetime) {
            throw new InvalidArgumentException('This DPD additional service is not documented for primetime parcels.');
        }
    }
}
